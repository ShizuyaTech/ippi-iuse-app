...
<head>
    ...
    <title>Daftar Kiriman Bahan</title>
    <style>
...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">DAFTAR KIRIMAN BAHAN DARI IPPI</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-family: monospace; font-size:8px; color:#0f766e; font-weight:bold;">{{ $r->vmd_number }}</td>
                <td>{{ $r->delivery_date?->format('d/m/Y') ?? '-' }}</td>
                <td>{{ $r->vehicle_number ?? '-' }}</td>
                <td>{{ $r->driver_name ?? '-' }}</td>
                <td style="text-align:right; font-weight:bold;">{{ $r->items->count() }}</td>
                <td><span class="badge badge-{{ $r->status }}">{{ $r->statusLabel() }}</span></td>
                <td style="color:#6b7280;">{{ $r->confirmed_at?->format('d/m/Y') ?? '-' }}</td>
            </tr>
            @empty
    ...
    <div class="footer">
        Dicetak dari IPPI iUse System &mdash; {{ now()->format('d/m/Y H:i') }} &mdash; Total: {{ $receipts->count() }} kiriman
    </div>
</div>
</body>
</html>
