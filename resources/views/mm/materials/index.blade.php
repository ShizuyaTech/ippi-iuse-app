        ...
        <table id="data-table" class="w-full text-sm border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="px-4 py-2 font-mono text-blue-700">{{ $m->code }}</td>
                    <td class="px-4 py-2">{{ $m->name }}</td>
                    <td class="px-4 py-2"><span class="px-2 py-0.5 rounded text-xs {{ $m->type==='RM'?'bg-gray-100 text-gray-700':''.($m->type==='WIP'?'bg-yellow-100 text-yellow-700':'bg-green-100 text-green-700') }}">{{ $m->type }}</span></td>
                    <td class="px-4 py-2">{{ $m->unit_of_measure }}</td>
                    <td class="px-4 py-2 text-right">{{ $m->qty_per_case > 0 ? number_format($m->qty_per_case, 0) : '-' }}</td>
                    <td class="px-4 py-2 text-right font-medium {{ $totalStock > 0 ? 'text-green-700' : 'text-gray-400' }}">
