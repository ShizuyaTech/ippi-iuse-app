<x-app-layout>
    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <h2 class="text-lg font-semibold text-gray-700 mb-4">Tambah Material Baru</h2>
        <form method="POST" action="{{ route('mm.materials.store') }}" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Kode Material *</label>
                    <input type="text" name="code" value="{{ old('code') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required maxlength="20">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tipe *</label>
                    <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                        <option value="RM" {{ old('type')=='RM'?'selected':'' }}>RM - Bahan Baku</option>
                        <option value="WIP" {{ old('type')=='WIP'?'selected':'' }}>WIP - Semi Jadi</option>
                        <option value="FP" {{ old('type')=='FP'?'selected':'' }}>FP - Produk Jadi</option>
                    </select>
                </div>
            </div>
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="unit_of_measure" value="{{ old('unit_of_measure', 'PCS') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Harga Standard *</label>
                    <input type="number" name="standard_price" value="{{ old('standard_price', 0) }}" step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Qty per Case / Karton</label>
                    <input type="number" name="qty_per_case" value="{{ old('qty_per_case', 0) }}" step="0.001" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="min_stock" value="{{ old('min_stock', 0) }}" step="0.001" min="0" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="order_method" id="order_method" onchange="toggleVendor(this.value)"
                            ...
                            <option value="mrp" {{ old('order_method','mrp')=='mrp'?'selected':'' }}>MRP (Perencanaan Bulanan)</option>
                ...
                <div>
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2 cursor-pointer">
                        <input type="checkbox" id="is_vendor_process" onchange="toggleProcessVendor(this.checked)"
                               {{ old('process_vendor_id') ? 'checked' : '' }}
                               class="w-4 h-4 rounded border-gray-300 text-blue-600">
                        Diproses di Vendor (WIP / FP)
                    </label>
                    <div id="process_vendor_field" class="{{ old('process_vendor_id') ? '' : 'hidden' }}">
                        <select name="process_vendor_id" id="process_vendor_select" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                            ...
                            @foreach($vendors as $v)
                            <option value="{{ $v->id }}" {{ old('process_vendor_id')==$v->id?'selected':'' }}>{{ $v->code }} — {{ $v->name }}</option>
                            @endforeach
                        </select>
