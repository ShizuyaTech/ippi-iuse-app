        ...
        <form method="POST" action="{{ route('mm.materials.update', $material) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $material->code) }}" class="w-full border rounded px-3 py-2 text-sm" required maxlength="20">
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $material->name) }}" class="w-full border rounded px-3 py-2 text-sm" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full border rounded px-3 py-2 text-sm">{{ old('description', $material->description) }}</textarea>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Unit of Measure *</label>
                    <input type="text" name="unit_of_measure" value="{{ old('unit_of_measure', $material->unit_of_measure) }}" class="w-full border rounded px-3 py-2 text-sm" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Harga Standard *</label>
                    <input type="number" name="standard_price" value="{{ old('standard_price', $material->standard_price) }}" step="0.01" min="0" class="w-full border rounded px-3 py-2 text-sm" required>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Qty per Case / Karton</label>
                    <input type="number" name="qty_per_case" value="{{ old('qty_per_case', $material->qty_per_case) }}" step="0.001" min="0" class="w-full border rounded px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="min_stock" value="{{ old('min_stock', $material->min_stock) }}" step="0.001" min="0" class="w-full border rounded px-3 py-2 text-sm">
            ...
            <div class="border rounded p-4 bg-gray-50 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div id="vendor_field" class="{{ old('order_method',$material->order_method)=='skm' ? '' : 'opacity-40 pointer-events-none' }}">
