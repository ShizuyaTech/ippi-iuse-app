<x-app-layout>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form method="GET" class="flex flex-wrap gap-2 mb-4 print:hidden">
            ...
            <label class="flex items-center gap-1.5 border rounded px-3 py-2 text-sm cursor-pointer {{ request('low_stock') ? 'bg-red-50 border-red-400 text-red-700 font-medium' : 'text-gray-600' }}">
        ...
        <table id="data-table" class="w-full text-sm border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="px-4 py-2 font-mono text-blue-700">{{ $s->material->code }}</td>
                    <td class="px-4 py-2">{{ $s->material->name }}</td>
                    <td class="px-4 py-2 text-xs"><span class="px-2 py-0.5 rounded bg-gray-100">{{ $s->material->type }}</span></td>
                    <td class="px-4 py-2">{{ $s->storageLocation->name }}</td>
                    @php
