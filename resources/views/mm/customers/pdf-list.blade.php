...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">DAFTAR CUSTOMER</div>
    ...
    <div class="filter-bar">
        ...
        &nbsp;|&nbsp; <span>Total:</span> {{ count($customers) }} customer
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-family: monospace; color: #1d4ed8;">{{ $c->code }}</td>
                <td style="font-weight: bold;">{{ $c->name }}</td>
                <td>{{ $c->contact_person ?? '-' }}</td>
                <td>{{ $c->email ?? '-' }}</td>
                <td>{{ $c->phone ?? '-' }}</td>
                <td class="center">
                    <span class="badge badge-{{ $c->is_active ? 'aktif' : 'nonaktif' }}">{{ $c->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                </td>
            </tr>
            @empty
    ...
    <div style="font-size:8px; color:#6b7280;">Total {{ count($customers) }} customer ditampilkan.</div>
