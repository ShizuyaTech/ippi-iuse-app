    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        ...
        <form method="POST" action="{{ route('mm.customers.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" maxlength="30">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                <textarea name="address" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">{{ old('address') }}</textarea>
            </div>
