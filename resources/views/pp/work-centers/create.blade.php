    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-xl">
        ...
        <form method="POST" action="{{ route('pp.work-centers.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required placeholder="WC001">
            </div>
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="capacity_per_hour" value="{{ old('capacity_per_hour') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" min="0" step="0.01">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Biaya per Jam (Rp)</label>
                    <input type="number" name="cost_per_hour" value="{{ old('cost_per_hour') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" min="0" step="0.01">
                </div>
            </div>
