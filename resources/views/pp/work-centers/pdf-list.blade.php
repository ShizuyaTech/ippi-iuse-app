...
<head>
    ...
    <title>Daftar Work Center</title>
    <style>
...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">DAFTAR WORK CENTER</div>
    ...
    <div class="filter-bar">
        ...
        &nbsp;|&nbsp; <span>Total:</span> {{ count($workCenters) }} work center
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Nama</th>
                <th>Deskripsi</th>
                <th class="right" style="width:13%">Kapasitas/Jam</th>
                <th class="right" style="width:13%">Biaya/Jam (Rp)</th>
                <th class="center" style="width:10%">Status</th>
            </tr>
        </thead>
        <tbody>
            @forelse($workCenters as $i => $wc)
            <tr>
                <td class="center">{{ $i + 1 }}</td>
                <td style="font-family: monospace; color: #1d4ed8;">{{ $wc->code }}</td>
                <td>{{ $wc->name }}</td>
                <td style="color:#6b7280;">{{ $wc->description ?? '-' }}</td>
                <td class="right">{{ number_format((float)($wc->capacity_per_hour ?? 0), 2) }}</td>
                <td class="right">{{ number_format((float)($wc->cost_per_hour ?? 0), 0, ',', '.') }}</td>
                <td class="center">
                    <span class="badge badge-{{ $wc->is_active ? 'aktif' : 'nonaktif' }}">{{ $wc->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                </td>
            </tr>
            @empty
    ...
    <div style="font-size:8px; color:#6b7280;">Total {{ count($workCenters) }} work center ditampilkan.</div>
