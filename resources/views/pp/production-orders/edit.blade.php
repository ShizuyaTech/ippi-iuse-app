        ...
        <form method="POST" action="{{ route('pp.production-orders.update', $productionOrder) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="bom_id" class="w-full border rounded px-3 py-2 text-sm" required>
                        @foreach($boms as $bom)
                ...
                <div>
                    ...
                    <select name="routing_id" class="w-full border rounded px-3 py-2 text-sm">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Qty Planned *</label>
                    <input type="number" name="quantity_planned" value="{{ old('quantity_planned', $productionOrder->quantity_planned) }}" class="w-full border rounded px-3 py-2 text-sm" min="0.001" step="0.001" required>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tgl Mulai Rencana</label>
                    <input type="date" name="planned_start_date" value="{{ old('planned_start_date', $productionOrder->planned_start_date?->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tgl Selesai Rencana</label>
                    <input type="date" name="planned_end_date" value="{{ old('planned_end_date', $productionOrder->planned_end_date?->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2 text-sm">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Catatan</label>
                <textarea name="notes" rows="2" class="w-full border rounded px-3 py-2 text-sm">{{ old('notes', $productionOrder->notes) }}</textarea>
            </div>
