...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">DAFTAR PRODUCTION ORDER</div>
    ...
    <div class="filter-bar">
        ...
        @if($filters['status'] ?? null) <span>Status:</span> {{ ucfirst(str_replace('-', ' ', $filters['status'])) }} &nbsp; @endif
        @if($filters['date_from'] ?? null) <span>Tgl Mulai &ge;:</span> {{ $filters['date_from'] }} &nbsp; @endif
        @if($filters['date_to'] ?? null) <span>Tgl Mulai &le;:</span> {{ $filters['date_to'] }} &nbsp; @endif
        @if(!($filters['search'] ?? null) && !($filters['status'] ?? null) && !($filters['date_from'] ?? null) && !($filters['date_to'] ?? null)) Semua data @endif
        &nbsp;|&nbsp; <span>Total:</span> {{ count($orders) }} order
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Nama Material</th>
                <th class="right" style="width:10%">Qty Plan</th>
                <th class="center" style="width:11%">Tgl Mulai</th>
                <th class="center" style="width:11%">Tgl Selesai</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-family: monospace; color: #1d4ed8;">{{ $order->order_number }}</td>
                <td style="font-family: monospace;">{{ $order->material->code ?? '-' }}</td>
                <td>{{ $order->material->name ?? '-' }}</td>
                <td class="right">{{ number_format((float)$order->quantity_planned, 3) }}</td>
                <td class="center">{{ $order->planned_start_date ? \Carbon\Carbon::parse($order->planned_start_date)->format('d/m/Y') : '-' }}</td>
                <td class="center">{{ $order->planned_end_date ? \Carbon\Carbon::parse($order->planned_end_date)->format('d/m/Y') : '-' }}</td>
                <td class="center">
    ...
    <div style="font-size:8px; color:#6b7280;">Total {{ count($orders) }} production order ditampilkan.</div>
