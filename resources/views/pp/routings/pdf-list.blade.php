...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title">DAFTAR ROUTING PRODUKSI</div>
    ...
    <div class="filter-bar">
        ...
        &nbsp;|&nbsp; <span>Total:</span> {{ count($routings) }} routing
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width:16%">No. Routing</th>
                <th style="width:14%">Kode Material</th>
                <th>Nama Material</th>
                <th class="center" style="width:7%">Tipe</th>
                <th class="center" style="width:12%">Jml Operasi</th>
                <th class="center" style="width:10%">Status</th>
            </tr>
        </thead>
        <tbody>
            @forelse($routings as $i => $routing)
            <tr>
                <td class="center">{{ $i + 1 }}</td>
                <td style="font-family: monospace; color: #1d4ed8;">{{ $routing->routing_number }}</td>
                <td style="font-family: monospace;">{{ $routing->material->code ?? '-' }}</td>
                <td>{{ $routing->material->name ?? '-' }}</td>
                <td class="center">
                    <span class="badge badge-{{ strtolower($routing->material->type ?? '') }}">{{ $routing->material->type ?? '-' }}</span>
                </td>
                <td class="center">{{ $routing->operations_count }}</td>
                <td class="center">
                    <span class="badge badge-{{ $routing->status }}">{{ ucfirst($routing->status) }}</span>
                </td>
            </tr>
            @empty
    ...
    <div style="font-size:8px; color:#6b7280;">Total {{ count($routings) }} routing ditampilkan.</div>
